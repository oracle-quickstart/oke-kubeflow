resource "oci_containerengine_cluster" "oke_kubeflow_cluster" {
...
resource "oci_containerengine_node_pool" "kubeflow_node_pool" {
  cluster_id         = var.create_new_oke_cluster ? oci_containerengine_cluster.oke_kubeflow_cluster[0].id : var.existing_oke_cluster_id
  compartment_id     = var.compartment_ocid
  kubernetes_version = var.kubernetes_version
