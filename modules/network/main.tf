resource "oci_core_vcn" "kubeflow_vcn" {
  count = var.useExistingVcn ? 0 : 1
  cidr_block     = var.VCN_CIDR
  compartment_id = var.compartment_ocid
  display_name   = "OKE Kubeflow VCN - ${random_string.deploy_id.result}" 
  dns_label      = var.vcn_dns_label
}

resource "oci_core_internet_gateway" "kubeflow_internet_gateway" {
  count = var.useExistingVcn ? 0 : 1
  compartment_id = var.compartment_ocid
  display_name   = "kubeflow_internet_gateway"
  vcn_id         = var.useExistingVcn ? var.custom_vcn[0] : oci_core_vcn.kubeflow_vcn.0.id
}
...
resource "oci_core_service_gateway" "kubeflow_service_gateway" {
  count = var.useExistingVcn ? 0 : 1
  compartment_id = var.compartment_ocid
  services {
  ...
  vcn_id = var.useExistingVcn ? var.custom_vcn[0] : oci_core_vcn.kubeflow_vcn.0.id
  display_name = "Kubeflow Service Gateway"
}
...
resource "oci_core_route_table" "RouteForComplete" {
  ...
  route_rules {
    ...
    network_entity_id = oci_core_internet_gateway.kubeflow_internet_gateway.*.id[count.index]
  }
}
...
#      destination       = var.oci_service_gateway
      ...
      network_entity_id = oci_core_service_gateway.kubeflow_service_gateway.*.id[count.index]
    }
...
resource "oci_core_subnet" "edge" {
  ...
  dhcp_options_id     = oci_core_vcn.kubeflow_vcn[count.index].default_dhcp_options_id
  dns_label           = "edge"
}
